"""
航大思考179 検証スクリプト

問1: 3属性独立変化のパターン認識（難度引き上げ版）
  列: 白円1, 黒三角1, 白四角2, 黒円3, 白三角5, 黒四角8, 白円13, ?
  規則(形): 円, 三角, 四角 の3周期循環
  規則(色): 白, 黒 の2周期循環
  規則(数): フィボナッチ数列 1,1,2,3,5,8,13,21
  → 8番目 = 黒三角・21

問2: フィボナッチ的な面積問題
  1辺1の正方形から、長辺を1辺とする正方形を加える操作を9回行う。
  → 面積 = F(10) × F(11) = 55 × 89 = 4895
"""


def verify_q1():
    """問1: 3属性独立変化パターン認識の一意性確認"""
    shapes = ["円", "三角", "四角"]
    colors = ["白", "黒"]

    # フィボナッチ数列
    fib = [1, 1]
    while len(fib) < 8:
        fib.append(fib[-1] + fib[-2])

    sequence = []
    for i in range(8):
        sequence.append((colors[i % 2], shapes[i % 3], fib[i]))

    print("問1: 列の構成")
    for i, (c, s, n) in enumerate(sequence, 1):
        marker = " ← ?" if i == 8 else ""
        print(f"  {i}: {c}{s}・{n}{marker}")

    expected_answer = ("黒", "三角", 21)
    assert sequence[7] == expected_answer, f"答が一致しません: {sequence[7]} != {expected_answer}"
    print(f"問1 答: {sequence[7][0]}{sequence[7][1]}・{sequence[7][2]}")

    # 選択肢で完全一致するのは(3)のみ
    options = [
        (1, "白", "三角", 21),  # 色のみ違い
        (2, "黒", "四角", 21),  # 形のみ違い
        (3, "黒", "三角", 21),  # ← 正解
        (4, "黒", "三角", 13),  # 数字のみ違い（1つ前のフィボナッチ）
        (5, "白", "円",  21),   # 形・色の両方違い
    ]
    matches = [n for n, c, s, num in options if (c, s, num) == expected_answer]
    assert matches == [3], f"選択肢が一意でない: {matches}"
    print(f"問1 一意解: 選択肢({matches[0]})\n")


def verify_q2():
    """問2: 操作9回後の長方形面積"""
    short, long_ = 1, 1
    print("問2: 操作ごとの長方形サイズ")
    print(f"  開始: {short}×{long_} (面積={short*long_})")

    for n in range(1, 10):
        new_short = long_
        new_long = short + long_
        short, long_ = new_short, new_long
        print(f"  {n}回目: {short}×{long_} (面積={short*long_})")

    area = short * long_
    print(f"\n問2 答: {area}")
    assert area == 4895, f"想定と異なる: {area}"

    fib = [0, 1, 1]
    for _ in range(15):
        fib.append(fib[-1] + fib[-2])
    expected = fib[10] * fib[11]
    print(f"  フィボナッチ検証: F(10)×F(11) = {fib[10]}×{fib[11]} = {expected}")
    assert area == expected

    options = {1: 4895, 2: 1870, 3: 2584, 4: 6765, 5: 3025}
    matches = [k for k, v in options.items() if v == area]
    assert matches == [1], f"選択肢が一意でない: {matches}"
    print(f"問2 一意解: 選択肢({matches[0]})\n")


if __name__ == "__main__":
    verify_q1()
    verify_q2()
    print("=== すべて検証完了 ===")
